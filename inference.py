...
def Visualize(pred_path, frame_path, output_path):
    total_dist = 0
    ...
    # TODO: convert to script for pipeline
    count = 0
    ...
    print(f"[RES] average distance from target: {int(total_dist)}")


def OutputToGif(output_path):
    frames = []
    ...
    frames[0].save(output_path+f'{int(time.time())}.gif', format='GIF',
                   append_images=frames[1:],
                   save_all=True,
                   duration=200, loop=0)


def OutputToVideo(output_path):
    frames = []
    imgs = glob.glob(output_path + "*.png")
    imgs.sort(key=lambda f: int(re.sub('\D', '', f)))
    # print(imgs)
    ...
    clip.write_videofile(output_path+'my_video.mp4')




if __name__ == '__main__':
    Visualize("runs/detect/test4/labels/", "bbox_save/", "script_out/")
    OutputToGif("script_out/")
    OutputToVideo("script_out/")